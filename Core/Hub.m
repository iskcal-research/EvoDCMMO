    ...
    methods
        ...
        function pop = GetIndis(obj, varargin)
            if isempty(obj.result_pop) || obj.g > obj.result_pop{end, 1}
                ...
                fits = obj.problem.GetFits(decs); 
                cons = obj.problem.GetCons(decs); 
                if ~isempty(adds)
...
%                     obj.perform(end+1).all = size(obj.problem.GetBestPos(), 1);
%                     obj.perform(end).found = obj.problem.GetFoundPeak(obj.algorithm.Record(), 1e-3);
        ...
        function perform = Start(obj)
            ...
            while(~obj.algorithm.Terminate())
                ...
                if ~isempty(obj.result_pop) && isempty(obj.result_pop{end, 3})
                    ...
                    obj.perform(end+1).all = size(obj.problem.GetBestPos(), 1);
                    obj.perform(end).found = obj.problem.GetFoundPeak(obj.algorithm.Record(), [1e-3 1e-4 1e-5]);
                    if isa(obj.problem, 'DynamicProblem') % the environment changes if the problem is dynamic
                        obj.problem.ChangeDynamic();
                    end
                end
...
%             perform = obj.perform_method(obj.result_pop, history_pos, history_fit);
%             obj.perform = perform;
            perform = obj.perform;
        end
